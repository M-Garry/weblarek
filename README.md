...
Тип `TPayment` определён как: type `TPayment = 'card' | 'cash' | null`
...
Методы:
`UpdateBuyer<K extends keyof IBuyer>(field: K, value: IBuyer[K]): void `— обновляет значение одного поля модели, указанного в параметре field. Остальные поля остаются без изменений.
...
`Validate(): { [K in keyof IBuyer]?: string }` — выполняет валидацию полей. Возвращает объект, в котором ключи — имена невалидных полей, а значения — тексты ошибок. Если поле валидно, оно отсутствует в результате. Поле считается невалидным, если:
payment равен null;
строковые поля (address, email, phone) пусты или содержат только пробелы.
## Слой коммуникации
...
`Get(): Promise<IProduct[]>` -  Выполняет GET-запрос к эндпоинту /product/ для получения полного списка товаров.
